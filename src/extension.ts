...
export function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('invisaline.enable', () => {
  ...
  function updateDecorations(editor: vscode.TextEditor | undefined) {
    ...
    const extraPad = vscode.workspace.getConfiguration('invisaline').get<number>('extraPad', 2);
    ...
    while ((match = templateRegex.exec(text))) {
      ...
      if (start.line < end.line) {
        ...
        for (let ln = start.line + 1; ln <= end.line; ln++) {
          ...
          if (padCount > 0) {
            const padStr = '\u00A0'.repeat(padCount + extraPad);
